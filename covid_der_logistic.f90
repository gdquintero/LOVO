program logistic
   ...
   subroutine compute_grad_sp(n,x,pdata,res)
      ...
      do i = 1, pdata%lovo_order
         ...
         gaux2 = 1.0d0 / (x(3) + x(1) * (exp(x(2) * ti) - 1.0d0))**3

         res(1) = res(1) + gaux1 * gaux2 * x(2) * (x(3)**2) * exp(x(2) * ti) * &
                  (x(3) - x(1) - x(1) * exp(x(2) * ti))

         res(2) = res(2) + gaux1 * gaux2 * x(1) * x(3) * (x(3) - x(1)) * &
                  exp(x(2) * ti) * (x(1) * exp(x(2) * ti) - ti * x(1) * x(2) * exp(x(2) * ti) - &
                  ti * x(1) * x(2) - x(1) + ti * x(2) * x(3) + x(3))

         res(3) = res(3) + gaux1 * gaux2 * (x(1)**2) * x(2) * exp(x(2) * ti) * (x(1) - x(3) - &
                  x(1) * exp(x(2) * ti) + 2.d0 * x(3) * exp(x(2) * ti))
      enddo
   ...
   subroutine model(n,x,i,pdata,res)
      ...
      res = x(1) * x(2) * x(3) * (x(3) - x(1)) * exp(x(2) * pdata%t(i))
      res = res / (x(3) + x(1) * (exp(x(2) * pdata%t(i)) - 1.0d0))**2
