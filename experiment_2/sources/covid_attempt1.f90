program main
    ...
    subroutine hard_test()
        ...
        pdata%xk(pdata%n),pdata%grad_sp(pdata%n),abs_err(5,n_test),av_abs_err(5),stat=allocerr)
            ...
            do n_train = 5,10,1
                ...
                do k = 1, n_train
                    ...
                    if (.not. ANY(outliers(1:noutliers) .eq. k)) then
                        av_err_train = av_err_train + absolute_error(covid_data(24+i+k-n_train),pred)
                    endif
                enddo  
               
                av_err_train = av_err_train / (n_train - noutliers)

                if (av_err_train .lt. av_err_train_best) then
                    optimal_ntrain = n_train
                    av_err_train_best = av_err_train
                endif
    ...
    !*****************************************************************
    !*****************************************************************
    real(kind=8) function cubic_model(tm,ym,pdata)
        implicit none

        real(kind=8) :: tm,ym
        type(pdata_type) :: pdata

        cubic_model = ym + pdata%xk(1) * (tm - 1.d0) + &
        pdata%xk(2) * ((tm - 1.d0)**2) + pdata%xk(3) * ((tm - 1.d0)**3)

    end function cubic_model
