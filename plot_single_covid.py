...
df_solution_cubic = pd.read_table("output/plot_covid_cubic.txt",delimiter=" ",header=None,skiprows=0,skipinitialspace=True)
...
for i in range(n):
    plt.plot(df_solution_cubic[0].values[i*h:i*h+h-1],df_solution_cubic[1].values[i*h:i*h+h-1])
